...
export interface SelectionResult {
   ...
   * On click event pass through.
   */
  onEdgeClick?: (data: GraphNode) => void;
...
## Edge Selection
Depending on the `PathSelectionTypes` and `SelectionTypes`, edges are also selected onNodeClick. If you don't wish to select edges onNodeClick for multi selections, set `PathSelectionTypes` to 'direct'.
